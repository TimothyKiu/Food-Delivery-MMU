...
deen_food = [
    ...
    {'name': 'AYAM + NASI', 'price': 6.00, 'category': 'Soups'},
    # Likely a mistake on the menu, assuming this should not be Vegetarian
    {'name': 'DAGING + NASI', 'price': 7.00, 'category': 'Soups'},
    # Likely a mistake on the menu, assuming this should not be Vegetarian
...
htc_food = [
    ...
    {'name': 'ROJAK MEE/MEE', 'price': 5.50, 'category': 'Goreng'},
    # Assuming this is a typo and should be "ROJAK MEE/BIHUN"
...
def bakery():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.commit()
            cart[item_name] = {'quantity': existing_item[3] + quantity,
                               'total_price': existing_item[2] * (existing_item[3] + quantity)} if existing_item else {
                'quantity': quantity, 'total_price': 0.0 * quantity}
...
def update_remarks():
    ...
    try:
        ...
        mycursor.execute("UPDATE Cart SET remarks = %s WHERE username = %s AND food_name = %s",
                         (remark, username, food_name,))
        db.commit()
    except mysql.connector.Error as err:
        print(f"Error: {err}")
        db.rollback()
